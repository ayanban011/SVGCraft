...
def get_masked_latents_all_list(
    ...
    **kwargs,
    ...
    for idx, ((prompt, phrase, word, box), input_latents) in enumerate(
        ...
        #torch.save(latents_all_list, 'latents_all.pth')
        #torch.save(mask_tensor_list, 'masked_latents.pth')
    return latents_all_list, mask_tensor_list, saved_attns_list, so_img_list
...
def run(
    ...
    with torch.autocast("cuda", enabled=use_autocast):
        ...
        _, images, attentionmap = pipelines.generate_gligen(
            model_dict,
            composed_latents,
    ...
    utils.free_memory()
    return EasyDict(image=images[-1], so_img_list=so_img_list)
