...
def main(args):
    ...
    if args.use_blob:
        ...
        for a in range(32):
            ...
            for b in range(32):
                ...
                path_group = pydiffvg.ShapeGroup(shape_ids = torch.tensor([len(shapes) - 1]),
                                        stroke_color = torch.tensor([0.3,0.6,0.3,1.0]),#green
                                        fill_color = None)
                shape_groups.append(path_group)
    ...
    img = render(canvas_width, # width
                 ...
                 None,
                 *scene_args)
    pydiffvg.imwrite(img.cpu(), 'final_sketch/painterly_batman/init.png', gamma=gamma)
    ...
    for t in range(args.num_iter):
        ...
        pydiffvg.imwrite(img.cpu(), 'final_sketch/painterly_batman/iter_{}.png'.format(t), gamma=gamma)
        img = img[:, :, :3]
        ...
        if args.use_blob:
            ...
            for group in shape_groups:
                ...
                group.stroke_color.data[-1].clamp_(0.0, 0.3) # opacity

        if t % 10 == 0 or t == args.num_iter - 1:
            pydiffvg.save_svg('final_sketch/painterly_batman/iter_{}.svg'.format(t),
                              canvas_width, canvas_height, shapes, shape_groups)
    ...
    img = render(target.shape[1], # width
                 ...
                 *scene_args)
    # Save the intermediate render.
    pydiffvg.imwrite(img.cpu(), 'final_sketch/painterly_batman/final.png'.format(t), gamma=gamma)
    # Convert the intermediate renderings to a video.
    from subprocess import call
    call(["ffmpeg", "-framerate", "24", "-i",
        "final_sketch/painterly_batman/iter_%d.png", "-vb", "20M",
        "final_sketch/painterly_batman/out.mp4"])
