...
def main(args):
    ...
    img = render(canvas_width, # width
                 ...
                 None,
                 *scene_args)
    pydiffvg.imwrite(img.cpu(), 'painterly_rendering/init.png', gamma=gamma)
    ...
    for t in range(args.num_iter):
        ...
        pydiffvg.imwrite(img.cpu(), 'painterly_rendering/iter_{}.png'.format(t), gamma=gamma)
        img = img[:, :, :3]
        ...
        if t % 10 == 0 or t == args.num_iter - 1:
            pydiffvg.save_svg('final_sketch/painterly_rendering/iter_{}.svg'.format(t),
                              canvas_width, canvas_height, shapes, shape_groups)
    ...
    img = render(target.shape[1], # width
                 ...
                 *scene_args)
    # Save the intermediate render.
    #pydiffvg.imwrite(img.cpu(), 'supp_results/painterly_rendering/final.png'.format(t), gamma=gamma)
    # Convert the intermediate renderings to a video.
    from subprocess import call
    call(["ffmpeg", "-framerate", "24", "-i",
        "painterly_rendering/iter_%d.png", "-vb", "20M",
        "painterly_rendering/out.mp4"])
